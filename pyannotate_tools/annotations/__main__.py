...
parser.add_argument(
    "--type-info", default="type_info.json", metavar="FILE", help="JSON input file (default type_info.json)"
)
parser.add_argument("--uses-signature", action="store_true", help="JSON input uses a signature format")
parser.add_argument("-p", "--print-function", action="store_true", help="Assume print is a function")
parser.add_argument("-w", "--write", action="store_true", help="Write output files")
parser.add_argument(
    "-j", "--processes", type=int, default=1, metavar="N", help="Use N parallel processes (default no parallelism)"
)
parser.add_argument(
    "--max-line-drift",
    type=int,
    default=5,
    metavar="N",
    help="Maximum allowed line drift when inserting annotation" " (can be useful for custom codecs)",
)
parser.add_argument("-v", "--verbose", action="store_true", help="More verbose output")
parser.add_argument("-q", "--quiet", action="store_true", help="Don't show diffs")
parser.add_argument(
    "-d", "--dump", action="store_true", help="Dump raw type annotations (filter by files, default all)"
)
parser.add_argument(
    "-a", "--auto-any", action="store_true", help="Annotate everything with 'Any', without reading type_info.json"
)
parser.add_argument("files", nargs="*", metavar="FILE", help="Files and directories to update with annotations")
parser.add_argument("-s", "--only-simple", action="store_true", help="Only annotate functions with trivial types")
parser.add_argument(
    "--python-version",
    action="store",
    default="2",
...
class ModifiedRefactoringTool(StdoutRefactoringTool):
    ...
    def refactor_file(self, filename, write=False, doctests_only=False):
        try:
            super(ModifiedRefactoringTool, self).refactor_file(filename, write=write, doctests_only=doctests_only)
...
def main(args_override=None):
    ...
    if args.auto_any:
        ...
        if args.uses_signature:
            ...
            data = generate_annotations_json_string(args.type_info, only_simple=args.only_simple)
    ...
    rt = ModifiedRefactoringTool(
        fixers=fixers, options=flags, explicit=fixers, nobackups=True, show_diffs=not args.quiet
    )
    if not rt.errors:
