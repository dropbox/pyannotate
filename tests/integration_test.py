    ...
    def test_no_type_info(self):
        ...
        try:
            subprocess.check_output(
                [sys.executable, "-m", "pyannotate_tools.annotations", "gcd.py"], stderr=subprocess.STDOUT
            )
            assert False, "Expected an error"
        except subprocess.CalledProcessError as err:
            assert err.returncode == 1
            lines = err.output.splitlines()
            assert b"Can't open type info file: " b"[Errno 2] No such file or directory: 'type_info.json'" in lines
